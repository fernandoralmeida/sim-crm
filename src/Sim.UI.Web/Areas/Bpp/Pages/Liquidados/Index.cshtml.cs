...
namespace Sim.UI.Web.Areas.Bpp.Pages.Liquidados;
...
public class IndexModel : PageModel {
    ...
    public async Task OnGetAsync() {  
        LoadSelectors();      
        var _list = await _appcontratos.DoListAsync(s => s.AppUser == User.Identity!.Name && s.Situacao == EnSituacao.Aprovado && s.Pagamento == EnPagamento.Liquidado);      
    ...
    public async Task OnPostAsync(string src) {
        var _list = await _appcontratos.DoListAsync(s => s.AppUser == User.Identity!.Name && s.Situacao == EnSituacao.Analise);
